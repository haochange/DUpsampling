...
class ResNet_SSN(nn.Module):
    ...
    def forward(self, image_pyramid):
        ...
        x = self.relu2(self.bn2(self.conv2(scaled_feature)))  # 256 * 256
...
def SSN_Res(num_classes=21):
    model = ResNet_SSN(Bottleneck, [3, 4, 23, 3], num_classes)
    return model


class SSN_Solver(BaseModel):
    def __init__(self, opt, dataset=None):
        ...
        if self.opt.isTrain:
            ...
            shutil.copyfile(os.path.join(self.model_path, 'base_model.py'),
                            os.path.join(self.model_dir, 'base_model.py'))
